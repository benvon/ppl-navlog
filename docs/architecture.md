...
The Worker is independent of `runway-picker`. Future airport and METAR adapters may call a compatible `runway-picker` contract through a configurable internal or HTTPS adapter, but a direct authoritative-upstream adapter remains available for local development and recovery. No Worker secret is available to browser JavaScript.
...
This Worker exposes stable navlog routes `GET /api/airports/:icao` and `GET /api/weather/metar/:icao`, validates the evidenced payloads before mapping them, and rejects mismatched resource identity. It can use an optional `RUNWAY_PICKER_API` service binding with a fixed configuration-owned origin. No binding is declared or deployed in this tranche, so missing configuration returns a structured 503 response and direct upstream fallback remains a future adapter. Winds has only a schema seam; no winds provider or live adapter has been selected.
