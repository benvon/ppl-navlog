...
The issue #7 planner's actions and session state stay in `src/ui/pilot-intent-planner.ts`. It renders pilot input controls and the calculated worksheet and inspector after a successful update. Pilot input persistence is handled through `PilotInputRepository`; airport and weather retrieval and calculation run for the explicit **Update plan** action. The result and its provenance are in memory for the current session only. Editing inputs, a failed update, opening a plan, importing a plan, or reloading leaves no current result.
