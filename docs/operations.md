...
The development domain is `navlog.benvon.dev`; the planned production domain is `navlog.benvon.net`. See `docs/github-ci-cd-setup.md` for GitHub and Cloudflare configuration, `docs/release-readiness-audit.md` for milestone status, and `docs/security-review.md` for production blockers.

The Worker serves a prebuilt static artifact and exposes `GET /api/health`. Its non-sensitive response contains status, build version, commit SHA, and request ID. The UI displays a teaching-only disclaimer above the workspace and the static build identity in the page footer. Deployment requires the `RUNWAY_PICKER_API` service binding target `runway-picker-metar-api` in the same account; local mock tests do not validate that live binding.
