...
The Worker serves a prebuilt static artifact and exposes `GET /api/health`. Its non-sensitive response contains status, build version, commit SHA, and request ID. The UI displays the static build identity. Deployment requires the `RUNWAY_PICKER_API` service binding target `runway-picker-metar-api` in the same account; local mock tests do not validate that live binding.
