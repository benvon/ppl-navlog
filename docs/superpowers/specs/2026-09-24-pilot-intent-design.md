...
Plan and aircraft-profile import/export are explicitly out of scope before version 1.0. Keep pilot inputs and profiles in the current browser's local store; do not add recovery files or portability controls. Printing and PDF output are deferred while core planning functionality is built. The page shows a teaching-only disclaimer above the planner and build identity in the footer.
...
2. The planner presents one primary **Update plan** action. It is disabled while locally required inputs are missing or invalid, including malformed airport identifiers and routes with more than 25 checkpoints. Nearby controls explain specific problems. External data availability does not determine this local enablement. The handler repeats local validation before doing work. Invalid raw text still autosaves as a working copy.
...
- Do not implement plan or profile import/export in the pre-1.0 release. Avoid storing sensitive browser data in a server or request to a new endpoint; text rendering uses `textContent`.
...
- The 25-checkpoint limit is enforced in the editor, local Update gate, and storage validation. Malformed airport codes cannot create an Update submission or trigger lookup, though their literal text is saved on blur.
- The page-wide teaching disclaimer is visible above the workspace, build identity is in the footer, and the active planner exposes no print action.
- Update plan is locally gated and revalidates on invocation. Its submitted checkpoint persists through network or calculation failure; a failed input write prevents calculation. An old result is absent after failure or reload. Error persists through edits and retry start, then clears on successful update.
...
- Plan and profile import/export controls and recovery formats are absent. Weather refresh/recalculation does not add a pilot input checkpoint. Old v1 browser data is not loaded or replayed.
