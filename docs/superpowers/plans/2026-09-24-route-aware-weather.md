...
**Architecture:** The Worker owns aloft product selection, exact IATA station matching, and point interpolation, and exposes a separate bounded TAF read. The browser calculates route intervals progressively. Each point's single aloft answer applies to the interval that starts there; the next point's answer starts the following interval and cannot change a completed interval. Pilot waypoints and generated TOC/TOD points each receive one request. Carry UTC, fuel, altitude, position, and other calculated state forward, and assemble the navlog from completed intervals without a final whole-route recalculation. TOD placement uses planned TAS and descent rate without wind; actual descent uses the weather fetched at TOD and must reach the destination target altitude. Pilot inputs remain the only durable plan data.
...
- Winds product station IDs are IATA codes. Match only the exact catalog IATA ID; never interpret or fall back to ICAO, FAA, or aliases. Reject the entire product on a repeated product station ID or conflicting catalog identity. An ID without an exact IATA catalog match remains unavailable.
- Departure uses the selected METAR. Select the destination TAF group at TOD from projected arrival, then apply it to the terminal/pattern row, including worst-case applicable TEMPO/PROB winds. Every TAF group carries a complete start/end validity interval. Validate that completed arrival remains in the same full group window; block without recalculation if it does not. No runway selection or crosswind calculation.
- Make one aloft call per pilot waypoint and generated TOC/TOD. Each point's report is used for the following interval only. Calculate the full interval with that point's report, carry its end state forward, and never use the next point's weather to alter it. An arrival estimate may set the next request time only. Do not run a final calculation over a subset or the entire route.
- Place TOD using planned true airspeed and descent rate without wind. Use the weather returned at TOD for actual descent; fail with an explanation if destination target altitude cannot be reached.
...
- Product IDs are matched only to exact catalog IATA IDs; a duplicate product ID or conflicting catalog identity rejects the entire product. An ID without an exact catalog IATA match remains unavailable and is never resolved through another identifier field. Task 1 tests pin this behavior.
...
- TEMPO/PROB groups include complete start/end times and apply when projected arrival at TOD lies within the group interval; completed arrival must remain in that same full group window. Task 3 and Task 5 tests pin window boundaries and worst-case selection.
- A later point's weather cannot change a completed interval; Task 5 tests pin strict progression and state carry-forward.
...
`worker/api/winds.ts` owns product loading, catalog matching, and aloft resolution; `worker/api/taf.ts` owns the TAF upstream adapter and normalization; `worker/api/request.ts` and `handlers.ts` expose both routes. `src/services/weather/winds-client.ts` validates both response contracts. `src/application/route-weather-sampling.ts` owns sequential route-point requests and progressive interval calculation with carried state; it must not run a final recalculation or coverage pass. `src/application/arrival-taf-wind.ts` owns conditional-group selection. `src/application/full-navlog-engine.ts` consumes completed interval results without recalculating them. `src/ui/pilot-intent-planner.ts` owns the two endpoint source fields and current-result lifetime; `src/ui/calculation-inspector.ts` renders the retained math. Documentation updates belong with the task that changes the behavior.
...
- [ ] **Step 1: Add red identity and contract tests.** Include a product with a repeated station ID, a catalog where two entries claim the exact IATA ID with different coordinates, one where the product ID is absent from the catalog IATA field but appears in another identifier field, one where the ID belongs to another region, and unique IDs from each supported region. Expect the whole product to fail for duplicate product IDs or conflicting catalog identities. An ID without an exact IATA catalog match must remain unmatched and must never be resolved through another identifier field. Add a compile-time test of the point method shape.
...
- [ ] **Step 3: Replace the `stationInfo` last-entry-wins map with exact IATA match accounting.** Match only `iataId` by exact equality. Count every catalog match for each product ID, compare coordinates and region, reject the entire product if it repeats an ID or a reported ID has conflicting catalog identities, and never inspect alternate identifier fields. Preserve the existing unavailable behavior for IDs with no exact IATA catalog match. Add only the contract and method signature needed by Task 2; the method may throw a typed `service_unavailable` until Task 2 implements it.
...
### Task 5: Progressive point weather and route-interval calculation
...
**Interfaces:** Consumes `AloftPointAnswer`, `TafAnswer`, and endpoint METAR. Produces a progressive route solution containing completed intervals, the answer for each pilot waypoint and generated TOC/TOD point, per-interval evidence, and the final carried route state. Each point's answer is used only from that point forward. A completed interval is immutable before the next point query. The final navlog is assembled from these completed intervals and is not recalculated.

- [ ] **Step 1: Write red tests.** Assert exactly one point request per pilot waypoint and generated TOC/TOD, sequentially, with no repeated/final requests. Verify interval A→B uses only A's answer even when B's answer differs, and that B's answer affects only B→next. Verify position, altitude, UTC, fuel, and other state carry forward without recomputation. Cover TOC placement, fixed-TAS/rate no-wind TOD placement, actual TOD descent using TOD weather, and failure when the destination altitude cannot be reached. Assert request UTCs progress from calculated interval arrivals; a report that excludes its request or interval arrival stops the route before subsequent calls. Assert the bound is 29 requests for 25 checkpoints plus departure/destination/TOC/TOD. Use deferred fake responses to prove each interval completes before the following weather call starts.
...
- [ ] **Step 3: Implement** ordered route-event expansion and progressive calculation. At each current point, fetch its aloft answer once, fully calculate the interval to the next point using only that answer and profile/per-leg TAS/fuel-flow inputs, then carry its completed end state to the next point. Use a single arrival estimate only to set the next point request's planned UTC. Insert TOC at the planned climb-altitude crossing. Place TOD from descent altitude, planned TAS, and descent rate without wind; use the TOD point answer to calculate actual descent and stop with a clear error if target altitude is missed at destination. Fetch one point answer at each pilot waypoint and generated TOC/TOD. Never interpolate endpoint weather across a completed interval, recompute a completed interval, issue a final timing pass, or refetch. At TOD, select the destination TAF group from projected arrival, apply it to the terminal/pattern row, and choose the worst-case applicable TEMPO/PROB wind. Validate completed arrival against that same group's full validity interval; block without recalculating if it differs. Include per-interval source, report validity, and calculation math in inspector evidence.
...
- [ ] **Step 5: Architect review.** Independently verify a short and long route, exact API call counts, A-only interval behavior, carried end state, generated TOC/TOD placement, descent failure, and report-period failure; send any correction to this agent.
