...
**Scope decision:** Plan and aircraft-profile import/export are excluded from the pre-1.0 effort. Do not add recovery methods, UI, formats, or tests.
...
- [x] Make successful output session-only and clear on edit, failed attempt, reopen, or reload. Preserve error until a later success. Revalidate explicit period against current availability.
