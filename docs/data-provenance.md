...
Saved plans are immutable revisions. Refreshing weather or recalculating from changed inputs creates a new revision linked to its predecessor rather than silently changing an existing plan. The browser retains raw weather only when needed to explain a selected plan or troubleshoot a result. Exported data is versioned, validated on import, and excludes credentials.
