...
| --- | --- | --- |
| High | Public callers could drive upstream requests and exhaust provider capacity or incur cost. A per-source limit is not a global upstream quota. | Development config now binds `API_RATE_LIMITER` at 60 requests/minute and fails closed when unavailable. This accommodates the supported route's 37-request maximum including airport lookups and primary-to-alternate report fallback. **Still open for production:** verify live 429 behavior, add a separate production namespace, and apply Cloudflare WAF/bot controls. |
