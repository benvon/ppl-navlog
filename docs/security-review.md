...
| --- | --- | --- |
| High | The API rate limiter in `worker/index.ts` is optional and `wrangler.jsonc` has no `API_RATE_LIMITER` binding. Public callers could drive upstream requests and exhaust provider capacity or incur cost. A per-source limit is also not a global upstream quota. | **Open deployment blocker.** Add a rate-limit binding with distinct namespace IDs for dev and production, set a conservative policy, verify 429 behavior on each deployed Worker, and apply Cloudflare WAF/bot rules appropriate for a public API. |
