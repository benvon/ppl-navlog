...
Coordinates are taken from the exact station-catalog fixtures in `worker/api/winds.test.ts`. Pair distances use great-circle distance rounded to the nearest nautical mile. The initial 200 NM point-coverage limit supports the illustrated Hawaii island gaps and rejects the midpoint of the sparse FAI–BRW Alaska gap. The CONUS fixture is too sparse to demonstrate regional coverage between its stations; point requests still require a reporting station inside the limit. Sampling guidance is at most 100 NM between route point queries, subject to later representative-route validation.
